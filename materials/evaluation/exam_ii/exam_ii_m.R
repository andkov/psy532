...
exam_ii_results <- read.csv("C:/Users/koval_000/Google Drive/Courses/UVic-PSY-532-Univariate-GLM/materials/exams/exam_ii/exam_ii_results.csv")

ds <- exam_ii_results
# names(ds[1])
ds <- ds[!ds$name=="Andriy",]
ds <- ds[, names(ds) !="order"]
# ds <- ds[-1] # remove the name


## @knitr tweak_data
# head(ds) 
ds$total <- rowSums(ds[,names(ds)!="name"]) # compute the total
# head(ds)
...
## @kntir basic_table
dsL <- tidyr::gather(ds, name, score, 1:15)
