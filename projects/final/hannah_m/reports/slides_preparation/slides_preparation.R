...
# 
# 
# mInfo1 <- summary(lm.fit1)
# # logLik1 <- logLik(lm.fit1)
# dev1 <- deviance(lm.fit1)
# AIC1 <- AIC(lm.fit1)
# BIC1 <- BIC(lm.fit1)
# mInfo1
# 
# 
# mInfo1 <- c(dev1, AIC1, BIC1)
# 
# mInfo2 <- summary(lm.fit2)
# # logLik2 <- logLik(lm.fit2)
# dev2 <- deviance(lm.fit2)
# AIC2 <- AIC(lm.fit2)
# BIC2 <- BIC(lm.fit2)
# 
# mInfo2 <-c(dev2, AIC2, BIC2)
# 
# #Save matrix as dataframe
# matrix = cbind(mInfo1, mInfo2)
# matrixtest=as.data.frame(matrix)
# 
# 
# f <- ggplot2::ggplot(matrixtest, aes("mInfo1")) + 
#     geom_point(size=4, shape=21, fill="white") + 
#     # geom_line(aes(y=value, group=variable, color=variable))+ 
#     theme1 
# f
# 
# plot(matrixtest$mInfo1)

# @knitr Plotmodelselection ------------------------
